...
impl Db {
    fn object_entry(&self) -> Entry<'_, bytes::Bytes, Object, ahash::RandomState> {
        self.entries.entry("a".into())
    }
}
...
impl Db {
    ...
    pub fn remove_object(&self, key: &Key) -> Option<(Key, Object)> {
        ...
        if let Some((_, obj)) = &mut entry {
            if let Some(obj_inner) = &obj.inner {
                self.update_expire_records(key, None, obj_inner.expire());
            }
    ...
    pub fn update_object(
        ...
        self.update_expire_records(key, new_ex, old_ex);
        Ok(())
    }
