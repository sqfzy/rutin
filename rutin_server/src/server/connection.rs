...
use rutin_resp3::codec::decode::{
    Resp3Decoder, decode_async, decode_async_multi2, decode_line_async,
};
use std::{
    any::Any,
...
pub struct Connection<St = TcpStream, B = StaticBytes, S = StaticStr>
where
    St: AsyncStream,
{
    pub stream: St,
    ...
    pub requests: VecDeque<Resp3<B, S>>,
}

impl<St: AsyncStream, B, S> Connection<St, B, S>
where
    B: for<'a> From<&'a mut [u8]> + Debug,
    S: for<'a> From<&'a mut str> + Debug,
    Resp3<B, S>: std::hash::Hash + Eq,
{
    pub fn new(stream: St, max_batch: usize) -> Self {
        Self {
            stream,
    ...
    pub async fn read_line(&mut self) -> RutinResult<&mut [u8]> {
        Ok(decode_line_async(&mut self.stream, &mut self.reader_buf).await?)
    }
    ...
    pub async fn read_frame(&mut self) -> RutinResult<Option<Resp3<B, S>>> {
    ...
    pub async fn try_read_frame(&mut self) -> RutinResult<Option<Resp3<B, S>>> {
        let mut decoder = Resp3Decoder::<B, S>::default();
    ...
    #[inline]
    #[instrument(level = "trace", skip(self), ret, err)]
    pub async fn read_frame_force(&mut self) -> RutinResult<Resp3<B, S>> {
        let frame = self
            .read_frame()
            .await?
    ...
    pub async fn write_frame<B2, S2>(&mut self, frame: &Resp3<B2, S2>) -> io::Result<()>
    where
        B2: AsRef<[u8]> + Debug,
        S2: AsRef<str> + Debug,
    {
