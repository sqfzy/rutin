...
def run_bench():
    ...
    for batch_size, num_requests in batch_size_to_num_requests.items():
        ...
        command_redis = f"redis-benchmark -r 1000 -p 6378 -t set -n {num_requests} -P {batch_size} -q"
        ...
        command_rutin = f"redis-benchmark -r 1000 -p 6379 -t set -n {num_requests} -P {batch_size} -q"
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "SET Command Throughput vs. Batch Size",
            "SET Command Latency vs. Batch Size",
        ),
    )
    ...
    fig.write_html(
        "result/html/bench_batch_throughput&latency_set.html",
    )
    fig.write_image(
        "result/svg/bench_batch_throughput&latency_set.svg",
        width=1400,
    )
