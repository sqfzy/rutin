...
​	Redis提供ACL控制命令执行以及键的访问。Rutin也提供了相似的支持，且配置的可读性更高。每个连接在初始化时，都会拥有自己的***Access Control***。如果设置了**default_ac**则所有连接初始化时都会使用该ac，如果设置了ACL（即存在其它ac配置）则所有连接初始化为最低权限（允许执行AUTH命令），如果没有配置任何ac则，所有连接会初始化为最高权限。当使用脚本执行命令时，脚本的权限与连接的权限是一致的。
