...
use crate::mapref::entry_ref::{EntryRef, OccupiedEntryRef, VacantEntryRef};
...
impl<'a, K: 'a + Eq + Hash, V: 'a, S: BuildHasher + Clone> DashMap<K, V, S> {
    ...
    pub fn entry_ref<'b, Q>(&'a self, key: &'b Q) -> EntryRef<'a, 'b, K, Q, V>
    where
    ...
    for DashMap<K, V, S>
{
    ...
    unsafe fn _get_read_shard(&'a self, i: usize) -> &'a HashMap<K, V> { unsafe {
        debug_assert!(i < self.shards.len());

        &*self.shards.get_unchecked(i).data_ptr()
    }}

    unsafe fn _yield_read_shard(&'a self, i: usize) -> RwLockReadGuard<'a, HashMap<K, V>> { unsafe {
        debug_assert!(i < self.shards.len());

        self.shards.get_unchecked(i).read()
    }}

    unsafe fn _yield_write_shard(&'a self, i: usize) -> RwLockWriteGuard<'a, HashMap<K, V>> { unsafe {
        debug_assert!(i < self.shards.len());
    ...
    unsafe fn _try_yield_read_shard(
        &'a self,
        i: usize,
    ) -> Option<RwLockReadGuard<'a, HashMap<K, V>>> { unsafe {
        debug_assert!(i < self.shards.len());
    ...
    unsafe fn _try_yield_write_shard(
        &'a self,
        i: usize,
    ) -> Option<RwLockWriteGuard<'a, HashMap<K, V>>> { unsafe {
        debug_assert!(i < self.shards.len());
    ...
    fn _entry_ref<'b, Q>(&'a self, key: &'b Q) -> EntryRef<'a, 'b, K, Q, V>
    where
        Q: Hash + equivalent::Equivalent<K> + ?Sized,
    {
        let hash = self.hash_u64(&key);
        ...
        match shard.find_or_find_insert_slot(
            ...
            Ok(elem) => EntryRef::Occupied(unsafe { OccupiedEntryRef::new(hash, shard, elem) }),
            Err(slot) => EntryRef::Vacant(unsafe { VacantEntryRef::new(shard, key, hash, slot) }),
        }
    }
